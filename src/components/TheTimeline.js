...
export const availableTimelines = writable([]);
...
function loadTimeline(info) {
  ...
  // On garde le id de la timeline en rérérence pour la sauvegarder plus tard
	// #RENDUICI: en fait oui... Faut load le ID si elle vient de la DB en tout case
	// #NEEDSTESTING
	...
	console.log('@info.start_date: ', info.end_date);
	 try {
		 ...
		 document.querySelector('.dateIndicator:nth-of-type(2)').innerHTML =info.end_date;
			...
			// Then we're probably on the homepage
		}
  // if (info.public == 0) {
  ...
  info.lines.forEach(function (line, lineIndex) {
    group.add({
      id: lineIDCount,
      content: line.name,
      order: line.order,
